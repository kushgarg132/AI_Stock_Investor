...
async def test_unconfigured_when_no_api_key_or_secret():
    manager = KiteSessionManager(api_key=None, api_secret=None, redis=_redis())
...
async def test_needs_login_when_configured_but_no_cached_token():
    manager = KiteSessionManager(api_key="key", api_secret="secret", redis=_redis(get_return=None))
...
async def test_active_when_cached_token_passes_profile_check():
    ...
    manager = KiteSessionManager(api_key="key", api_secret="secret", redis=redis)
...
async def test_generate_login_url_delegates_to_sdk():
    manager = KiteSessionManager(api_key="key", api_secret="secret", redis=_redis())
...
async def test_exchange_request_token_stores_access_token_with_expiry():
    ...
    assert args[0] == "kite:access_token"
    assert args[1] == "new-token"
    assert kwargs["ex"] > 0


async def test_exchange_request_token_unconfigured_raises():
    manager = KiteSessionManager(api_key="key", api_secret=None, redis=_redis())
    with pytest.raises(RuntimeError):
